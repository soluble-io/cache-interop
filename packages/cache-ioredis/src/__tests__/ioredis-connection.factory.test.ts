import { createIoRedisNativeConnection, createIoRedisConnection } from '../ioredis-connection.factory';
  ...
  describe('createNativeConnection', () => {
    describe('when dsn is invalid', () => {
      it('should throw a parse error', () => {
        expect(() => createIoRedisNativeConnection('invalid:/')).toThrow("Can't parse DSN, reason PARSE_ERROR");
      });
    });
  });
