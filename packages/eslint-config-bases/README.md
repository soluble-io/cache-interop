...
Opinionated eslint config for cache-interop
...
```bash
$ yarn add --dev eslint eslint-config-bases
```
...
module.exports = {
  ...
  extends: [
    ...
    "eslint-config-bases/graphql-schema",
    "eslint-config-bases/storybook",
    "eslint-config-bases/playwright",
...
> **Tip:** "eslint-config-bases/prettier" must be set at the end to disable any
> conflicting rules.
